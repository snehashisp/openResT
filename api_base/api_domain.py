...
same web domain. API domains house the tests (structure in api_test file) that will be executed.
{
	...
	"options": {
		"async":False 
		// setting async to true asynchronously executes all the tests request in the domain 
		(i.e. it does not wait for one of the test to complete to go to the next one). 
	}
	"test1": { ... },
	"test2": { ... },
	...
...
class ApiDomain:
	...
	def __init__(self, domain_name, domain):
		...
		self.options = domain.pop("options", {})
