...
import aiohttp
import asyncio
from api_utils import Response, ElapsedTimeTracer
...
class ApiTestExecutor:
    ...
    def execute_test(self, test, stats):
        ...
        responses = asyncio.run(self._request_executer(test))
        return self._response_creator(test, responses)

    def execute_domain(self, domain, stats):

        responses = []
        tests = domain.generate_tests()
        for test in tests:
            responses.append(self.execute_test(test, stats))
        return responses
